...
def write_wkt_edges_id(city, effect):
	# effect = 'negative' # positive | negative
	...
	with open(filename, 'r') as fid:
		for row in fid.readlines():
			...
			rank   = tokens[0]
			edges_by_ranks[rank] = []
			# for through the OD pairs
			for k in range(1,len(tokens)):
				od_pair = tokens[k].split(' ')
				o = int(od_pair[0])
				d = int(od_pair[1])
				edges = get_path_edges_id(rank, city, o, d)
				for eid in edges:
					if(eid not in edges_by_ranks[rank]):
						edges_by_ranks[rank].append(eid)
			...
			with open(filename, 'w') as fid:
				# write column headers
				fid.write('eid\n')
				for eid in edges_by_ranks[rank]:
					fid.write('%d ' % eid)
				fid.write('\n')
