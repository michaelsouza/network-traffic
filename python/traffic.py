...
class City(object):
    ...
    def read_nodes(city_name):
        filename = 'instances/' + city_name + '_nodes_algbformat.txt'
    ...
    def read_edges(city_name, nodes):
        filename = 'instances/' + city_name + '_edges_algbformat.txt'
    ...
    def __calc_dist_all(nodes, edges):
        ...
        for i in range(s.size):
            d[i] = City.calc_dist(nodes, s[i], t[i])
        return d
    ...
    @staticmethod
    def calc_dist(nodes, node_i, node_j):
...
class MatOD(object):
    def __init__(this, filename, city=None):
        ...
        if npaths < this.o.size :
            ...
            print '      zero_volume       = %5d (#path: %2.1f%% vol: %2.1f%%)  ' % (issues['zero_vol']['pth'], 100 * issues['zero_vol']['pth']/totPth, (100 * issues['zero_vol']['vol'])/totVol)
            print '      zero_length       = %5d (#path: %2.1f%% vol: %2.1f%%)' % (issues['zero_len']['pth'], 100 * issues['zero_len']['pth']/totPth, (100 * issues['zero_len']['vol'])/totVol)
            print '      too_slow(>120min) = %5d (#path: %2.1f%% vol: %2.1f%%)' % (issues['too_slow']['pth'], 100 * issues['too_slow']['pth']/totPth, (100 * issues['too_slow']['vol'])/totVol)
    ...
    def show_path(this, pid, zero_based=False, verbose=False):
        if(this.city is not None):
            ...
            print('  o[%5d] %.7lf, %.7lf' %(s + (not zero_based), this.city.nodes['lat'][s], this.city.nodes['lon'][s]))
            print('  d[%5d] %.7lf, %.7lf' %(t + (not zero_based), this.city.nodes['lat'][t], this.city.nodes['lon'][t]))
...
def comparing_changes(tabA, tabB):
...
if __name__== '__main__':
     ...
     hist(city.edges.capacity, distname=None)
#
#     # load matod
